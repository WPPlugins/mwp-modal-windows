<?php if ( ! defined( 'ABSPATH' ) ) exit;
	function wow_admin_menu_wmw() {	
		add_menu_page(WOW_WMW_NAME, __( WOW_WMW_NAME, "wmw"), 'manage_options', WOW_WMW_SLUG, 'wow_page_wmw', 'dashicons-images-alt2', null);
			
	}
	add_action('admin_menu', 'wow_admin_menu_wmw', 999);
	function wow_page_wmw() {
		global $wow_plugin;	
		$wow_plugin = true;
		include_once( 'partials/modal.php' );
		wp_enqueue_script(WOW_WMW_SLUG, plugin_dir_url(__FILE__) . 'js/script.js', array('jquery'), WOW_WMW_VERSION);wp_enqueue_style('wow-new-style', plugin_dir_url(__FILE__) . 'css/style.css'); 	
